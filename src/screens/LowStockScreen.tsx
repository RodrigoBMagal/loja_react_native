...
type Props = BottomTabScreenProps<MainTabParamList, 'LowStock'>;
...
const LowStockItem = ({ product, onEdit, onUpdateQty }: LowStockItemProps) => {
...
const LowStockScreen = ({ navigation }: Props) => {
  ...
  const sections: { title: string; color: string; data: Product[] }[] = [
                  ...
                  <View key={section.title}>
                    ...
                    {section.data.map(product => (
                      <LowStockItem
                        key={product.id}
                        product={product}
                        onEdit={(p) => navigation.getParent()?.navigate('AddEditProduct', { product: p, mode: 'edit' })}
